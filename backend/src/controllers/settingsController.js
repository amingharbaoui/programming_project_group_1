...
async function resetChecklistItems(req, res) {
  // Transactioneel: anders kan de DELETE slagen en de INSERT falen → 0 criteria over.
  const conn = await db.getConnection();
  try {
    await conn.beginTransaction();
    await conn.query("DELETE FROM checklist_items");
    await conn.query(`
...
async function resetDocumentTypes(req, res) {
  // Transactioneel: de drie stappen (FK losmaken, custom types wissen, vaste types herstellen) horen
  // samen te slagen of samen terug te draaien — anders blijft de configuratie half aangepast.
  const conn = await db.getConnection();
  try {
    await conn.beginTransaction();
    ...
    await conn.query("DELETE FROM document_soorten WHERE is_vast = 0");
...
async function deleteDocumentType(req, res) {
  const id = Number(req.params.id);
  if (!id) return fail(res, 400, "Ongeldig documenttype-id");
  // Transactioneel: documenten loskoppelen en het type verwijderen horen samen te slagen.
  const conn = await db.getConnection();
  try {
    await conn.beginTransaction();
    const [rows] = await conn.query("SELECT is_vast FROM document_soorten WHERE id = ? LIMIT 1", [id]);
    if (rows.length === 0) { await conn.rollback(); return fail(res, 404, "Documenttype niet gevonden"); }
    if (rows[0].is_vast) { await conn.rollback(); return fail(res, 409, "Een vast documenttype kan niet verwijderd worden"); }
    await conn.query("UPDATE documenten SET document_soort_id = NULL WHERE document_soort_id = ?", [id]);
    await conn.query("DELETE FROM document_soorten WHERE id = ?", [id]);
    await conn.commit();
